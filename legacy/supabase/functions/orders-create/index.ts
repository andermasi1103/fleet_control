...
import { notifyNewOrder } from "../_shared/notifications.ts";
...
Deno.serve((request) =>
  protectedEndpoint(request, "POST", async ({ admin, session }) => {
    ...
    // The insert has already committed. Notification errors are contained by
    // notifyNewOrder so an FCM outage cannot turn a valid order into a failure.
    await notifyNewOrder(admin, order);
