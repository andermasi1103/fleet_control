...
void main() {
  test('fleet map usa Fastify, Bearer y conserva el contrato de marcadores', () async {
    ...
    final drivers = await FleetLocationsDataSource(_api(adapter)).list(sessionToken: 'token');
  ...
  test('reports usa la ruta tipada Fastify y conserva filas simplificadas', () async {
    final adapter = _Adapter(body: {
      'type': 'orders',
      'rows': [
        {'Fecha': '2030-01-01T00:00:00.000Z', 'Local': 'Central', 'Estado': 'pendiente'},
      ],
      'count': 1,
    });
    final report = await ReportsDataSource(_api(adapter)).load(token: 'token', type: ReportType.orders);
...
class _Adapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(RequestOptions options, Stream<Uint8List>? requestStream, Future<void>? cancelFuture) async {
    request = options;
    return ResponseBody.fromString(jsonEncode(body), 200, headers: {'content-type': ['application/json']});
  }
  @override
