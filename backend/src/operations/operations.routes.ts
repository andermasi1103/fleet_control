...
export function registerOperationsRoutes(app: FastifyInstance, database: Database): void {
  ...
  app.get('/api/orders/:id', { preHandler: app.requireFleetSession }, async (request, reply) => {
    ...
    if (!allowed && s.roleCode === 'chofer') { const assigned = await database.query<Row>('SELECT 1 FROM public.gestiones WHERE pedido_id=$1::uuid AND chofer_usuario_id=$2::uuid', [order.id, s.userId]); const eligible = order.estado === 'pendiente' && s.companyId === order.empresa_id && (await database.query<Row>('SELECT 1 FROM public.usuario_locales ul JOIN public.locales l ON l.id=ul.local_id WHERE ul.usuario_id=$1::uuid AND ul.local_id=$2::uuid AND l.empresa_id=$3::uuid', [s.userId, order.local_id, order.empresa_id])).rowCount === 1; allowed = assigned.rowCount === 1 || eligible; }
  ...
  app.get('/api/driver/orders/available', { preHandler: app.requireFleetSession }, async (request, reply) => {if(!sessionOr401(request.fleetSession,reply))return;const s=request.fleetSession;if(s.roleCode!=='chofer'||!s.companyId)return forbidden(reply);const result=await database.query<Row>(`SELECT p.id AS order_id,l.nombre AS local,pd.nombre AS descripcion,p.destino,p.destino_latitud,p.destino_longitud,p.factura_solicitud,p.numero_contacto,p.prioridad,p.observaciones,p.created_at FROM public.pedidos p JOIN public.locales l ON l.id=p.local_id JOIN public.usuario_locales ul ON ul.local_id=p.local_id AND ul.usuario_id=$1::uuid JOIN public.usuarios u ON u.id=ul.usuario_id AND u.activo=true AND u.empresa_id=p.empresa_id JOIN public.roles r ON r.id=u.rol_id AND r.codigo='chofer' LEFT JOIN public.pedido_descripciones pd ON pd.id=p.descripcion_tipo_id LEFT JOIN public.gestiones g ON g.pedido_id=p.id WHERE p.empresa_id=$2::uuid AND p.estado='pendiente' AND g.id IS NULL ORDER BY p.created_at DESC`,[s.userId,s.companyId]);return noStore(reply).send({orders:result.rows});});
